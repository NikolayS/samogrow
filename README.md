...
The site also carries a separate **blog** at `blog.html`, currently featuring one shareable page per dated, severity-tagged field-test incident (×10 nutrient overdose, transplant-borne leafroller, camera-missed aphids, neem phototoxicity). Language selection follows every internal blog link. Evidence photos are served locally from `docs/img/incidents/`, and each incident links to its full RCA issue. The cost section has shareable `?market=us|eu|aliexpress` sourcing views, keeps every figure in rough USD, normalizes totals per growing spot, excludes electricity and optional AI/API usage, and separates price from capability. Market precedence is the `?market=` URL, then a remembered manual choice, then browser region/language inference, and finally U.S. retail. The AliExpress view warns that generic Tuya-style plugs/cameras need adapter work with the current software.
